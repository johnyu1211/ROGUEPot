...
export function drawRazorWindEffect(ctx: any, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    // Step 1: Gathering wind spirals at target
    for (let i = 0; i < 3; i++) {
    ...
    // Step 2: 2 Razor sickle blades flying in from opposite sides
    drawSickle(tx - 32, ty - 18, -Math.PI * 0.25, 0.95, 0.95);
    drawSickle(tx + 32, ty + 12, Math.PI * 0.75, 0.95, 0.95);
  } else if (step === 3) {
    // Step 3: X-Cutting Razor Collision on target
    ...
    // Step 4: Dispersing wind pressure wisps
...
export function drawSwordsDanceEffect(ctx: any, userPos: { x: number; y: number }, step: number = 1) {
  ...
  const drawSword = (cx: number, cy: number, rot: number, scale: number = 1.0, alpha: number = 1.0) => {
    ...
    // Glowing Blade (#F87171 / #EF4444)
    ctx.fillStyle = "#FEE2E2";
    ctx.strokeStyle = "#DC2626";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(0, -32); // Blade tip
    ctx.lineTo(4.5, -24);
    ctx.lineTo(4.5, 6);
    ctx.lineTo(0, 8);
    ctx.lineTo(-4.5, 6);
  ...
  if (step === 1) {
    // Step 1: 4 swords appear orbiting user's waist
    const offsets = [
      { ox: -36, oy: -8, rot: -0.2 },
      { ox: 36, oy: -8, rot: 0.2 },
      { ox: -18, oy: 16, rot: -0.4 },
    ...
    // Step 2: Swords spin fast upward around user with red aura
    ...
    // Step 3: Swords clash together above user's head with brilliant attack aura!
    ctx.fillStyle = "rgba(245, 158, 11, 0.25)";
    ctx.beginPath();
    ctx.arc(ux, uy - 32, 54, 0, Math.PI * 2);
    ctx.fill();

    // 2 crossed swords above head
    ...
    // Attack boost rising arrows (▲ ▲)
    ctx.fillStyle = "#EF4444";
    ctx.font = "bold 16px DungGeunMo";
    ctx.fillText("▲ ATK UP", ux - 30, uy - 64);
  } else if (step >= 4) {
    // Step 4: Swords dissolve into rising red power aura particles
    const sparks = [
...
export function drawCutEffect(ctx: any, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    // Step 1: Diagonal slash windup flash
    ...
    // Step 2: Crisp brilliant green-white razor slash
    ctx.strokeStyle = "#22C55E";
    ctx.lineWidth = 8;
    ctx.lineCap = "round";
    ctx.beginPath();
    ...
    // Severed flying leaves flying out from cut
    drawLeaf(tx - 24, ty + 12, 0.4, 8);
    drawLeaf(tx + 26, ty - 18, -0.6, 9);
    drawLeaf(tx + 12, ty + 24, 1.2, 7);
  } else if (step === 3) {
    // Step 3: Green star impact burst + fluttering leaves
...
export function drawGustEffect(ctx: any, start: { x: number; y: number }, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    ...
    // Step 4: Dispersing gentle breeze sparkles
    const sparkles = [
      { ox: -24, oy: -20, r: 2.8 },
      { ox: 22, oy: -16, r: 3.2 },
      { ox: -12, oy: 18, r: 2.5 },
...
export function drawWingAttackEffect(ctx: any, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    // Step 1: Wing emerges beside target
    drawWing(tx - 38, ty - 18, -Math.PI * 0.25, 0.9, 0.85);
  } else if (step === 2) {
    // Step 2: Sharp wing cleave slashing through defender
    ...
    // Step 3: Bright impact star + flying feathers
...
export function drawWhirlwindEffect(ctx: any, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    // Step 1: Rising ground dust rings at base
    for (let i = 0; i < 3; i++) {
    ...
    // Step 2 & 3: Towering spiral tornado column
    const layers = 8;
    ...
    // Step 4: Fading horizontal wind rings
    for (let i = 0; i < 3; i++) {
...
export function drawFlyEffect(ctx: any, start: { x: number; y: number }, target: { x: number; y: number }, step: number = 1) {
  ...
  if (step === 1) {
    // Step 1: Sky Launch — Vertical speed lines launching attacker into clouds
    for (let i = -2; i <= 2; i++) {
    ...
    // Step 2: High Sky Target Lock — Fast shadow over defender
    ctx.fillStyle = "rgba(15, 23, 42, 0.45)";
    ctx.beginPath();
    ctx.ellipse(tx, ty + 24, 32, 10, 0, 0, Math.PI * 2);
    ctx.fill();

    // Descending supersonic air trails
    for (let i = -1; i <= 1; i++) {
    ...
    // Giant vertical piercing spear trail
    ctx.strokeStyle = "#38BDF8";
    ctx.lineWidth = 14;
    ctx.lineCap = "round";
    ctx.beginPath();
