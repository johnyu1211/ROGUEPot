  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      if (channel instanceof TextChannel || channel instanceof NewsChannel) {
        // 1. Close (archive & lock) all previous active threads of this user in the server
        try {
          const userTerms = [
            interaction.user.username.toLowerCase(),
            (interaction.user.globalName || "").toLowerCase(),
            (interaction.user.displayName || "").toLowerCase(),
            interaction.user.id,
          ].filter(Boolean);
      ...
      const sessionNoticeEmbed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          (isKo
            ? "⚠️ **데이터 및 세션 유실 면책 고지**\n" +
              "• 봇 업데이트/재부팅, 네트워크 연결 불안정 또는 장시간 미조작 등으로 인해 진행 중이던 게임 데이터나 배틀 세션이 유실될 수 있으며, 이에 대해 개발자는 일절 책임을 지지 않습니다.\n\n" +
              "**📜 오픈소스 & 크레딧**\n" +
            ...
            : "⚠️ **Data & Session Loss Disclaimer**\n" +
              "• The developer assumes NO responsibility for any loss of game progress or active battle sessions caused by bot updates, restarts, network instability, or prolonged inactivity.\n\n" +
              "**📜 Open Source & Credits**\n" +
